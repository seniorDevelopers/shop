    ...
    return (
        <>
            <div className='globalContainer account-wrapper'>
                <h1>{!changeLanguage ? "Salom," : "Привет,"} {data?.username}</h1>

                <div className='settings'>
                    <div className='setting-top'>
                        <h1 className='setting-top__title'>{!changeLanguage ? "Qaydnoma sozlamalar" : "Настройки профиля"} </h1>
                        <div className='setting-top__btns'>
                            <button onClick={openModal} className='setting-top__btns__btn-blue'> {!changeLanguage ? "Tahrirlash" : "Редактировать"}</button>
                            <button onClick={toHome} className='setting-top__btns__btn'>{!changeLanguage ? "Chiqish" : "Выход"}</button>
                        </div>
                    </div>
                    ...
                    <div className='setting-body'>
                        ...
                        <div className='setting-body__titles-group'>
                            <div className='setting-body__titles-group__titles'>
                                <h2 className='setting-body__titles-group__titles__title'>{!changeLanguage ? "Ism" : "Имя"}</h2>
                                <h2 className='setting-body__titles-group__titles__title'>{!changeLanguage ? "Familiya" : "Фамилия"}</h2>
                                <h2 className='setting-body__titles-group__titles__title'>{!changeLanguage ? "Telefon raqam" : "Номер телефона"}</h2>
                                <h2 className='setting-body__titles-group__titles__title'>{!changeLanguage ? "Yetkazish manzili" : "Адрес доставки"}</h2>
            ...
            {open && <Modal children={<Edit closeModal={setOpen} />} />}
        </>
    )
}
